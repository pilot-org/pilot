...
class _DependGraph:
    def __init__(self, depth) -> None:
        self._depth = depth
        self._specs_by_func_name: Dict[str, Spec] = {}

    def register_spec(self, spec, func_name):
        if func_name in self._specs_by_func_name.keys():
            raise click.UsageError(
                f'It is not support that the same function({func_name}) is used currently'
            )
        self._specs_by_func_name[func_name] = spec
    ...
    def trace_and_add_depend(self,
        ...
        for i in range(ignore_depth_plus, self._depth):
            ...
            the_spec_used_this = self._specs_by_func_name.get(func_name)
...
class _AsyncCachedInfoGetter(_CachedInfoGetterBase):
    ...
    def get(self) -> Cached:
        ...
        async def load_value():
            async with self._lock:
                ...
                logger.debug('{} caches {}', self, value)
    ...
    def clean_cache(self, due_to: Optional[str] = None) -> None:
        # TODO: lock this
        because = '' if due_to is None else f'Because {due_to} is deleted. '
        if self._is_cached:
...
class _InfoId:
    ...
    def __hash__(self):
        ...
        return hash(
            (self._spec.__class__, self._obj_id, self._args, hash_kwargs))
    ...
    def find_getter(self, obj: Obj, info_getter_stores: Dict[int,
                                                             Any]) -> Getter:
        hash_id: int = hash(self)
        try:
            return info_getter_stores[hash_id]
        except KeyError:
            getter = _new_cached_info_getter(self._spec, obj, info_id=self)
            info_getter_stores[hash_id] = getter
            return getter
...
class _CachedInfoSpec(Generic[Obj, Cached]):
    ...
    def __set_name__(self, cls, name: str):
        ...
        self._graph.register_spec(self, name)
...
class _InfoGroupPropertyGetter(Generic[Entry]):
    ...
    def __get__(self, obj, cls):
        ...
        if not hasattr(self, '_entry'):
            self._entry = self._entry_cls(obj)
        return self._entry
